...
void OBJFile::buildFacesNormal()
{
  for(int i = 0; i < psurf->nfaces; i++) {
    Vector v0 = psurf->verts[psurf->faces[i].verts[0][0]];
    Vector v1 = psurf->verts[psurf->faces[i].verts[1][0]];
    Vector v2 = psurf->verts[psurf->faces[i].verts[2][0]];

    //! normals point into objects, are vertices stored in counter-clockwise?
    psurf->faces[i].normal = ((v1 - v0) % (v2 - v0)).unitvector();
  }
}
...
int OBJFile::processvn(char *line){
  ...
  nfields = sscanf(line, "vn %lf %lf %lf", &nrm[0], &nrm[1], &nrm[2]);
  if(nfields != 3){
...
int OBJFile::processv(char *line){
  ...
  nfields = sscanf(line, "v %lf %lf %lf", &(vtx[0]), &(vtx[1]), &(vtx[2]));
  if(nfields != 3){
