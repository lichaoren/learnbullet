   ...
   ~ProgressMeter()
    ...
    void update()
    {
       ...
       if( currentPercent > oldPercent || ( currentUsedTime > 0 && initial) )
       {
          ...
          cout << "\r" << title << "  " << formattedPercent(currentPercent) << " Time Used: " << formattedTime(currentUsedTime) << " Time Left: " << formattedTime(estimatedTimeRemaining) << "   estimated Total Time: " << formattedTime(estimatedTotalTime) << "                      " << flush ;
       }
    }
